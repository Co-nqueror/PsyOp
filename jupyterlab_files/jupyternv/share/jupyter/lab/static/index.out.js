...
export async function main() {
  ...
  if (!queuedFederated.includes('ipyslickgrid')) {
    try {
      let ext = require('ipyslickgrid/src/jupyterlab-plugin');
      for (let plugin of activePlugins(ext)) {
        register.push(plugin);
      }
    } catch (e) {
  ...
  if (!queuedFederated.includes('bamboolib')) {
    try {
      let ext = require('bamboolib/src/jupyterlab-plugin');
      for (let plugin of activePlugins(ext)) {
        register.push(plugin);
      }
    } catch (e) {
